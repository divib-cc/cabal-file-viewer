...
class Mesh {
    ...
    constructor(dataView: DataView, offset: Offset, bones: number, header: Header) {
        ...
        // console.log(`📊 顶点数: ${this.vertex_count}, 面数: ${this.face_count}`);
        // ✅ 重要修复：检查顶点数是否为0
        if (this.vertex_count === 0) {
            // console.warn(`⚠️ 网格 ${this.id.text} 顶点数为0，跳过几何数据读取`);
            this.chunk_id = readUint32(dataView, offset);
            this.influence_count = readInt16(dataView, offset);
            // console.log(`空网格 CID:${this.chunk_id.toString(16)}, 影响数:${this.influence_count}`);
            return; // 直接返回，不读取后续数据
        }

        // 检查是否是 Python 格式的特殊处理
        const pythonIDs = [0x0003F103]; // 需要跳过的格式
        if (pythonIDs.includes(header.magic) && this.vertex_count > 0) {
            const bytesToSkip = readUint16(dataView, offset);
            // console.log(`⚠️ Python格式，跳过 ${bytesToSkip} 字节`);
